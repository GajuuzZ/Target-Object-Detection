...
def get_bboxs(pred_hm, pred_wh, pred_offset, topK=1, norm_wh=True):
    ...
    if norm_wh:
        wh = wh * torch.tensor((output_w, output_h)).to(wh.device)

    clses = clses.reshape(b, topK, 1)
    scores = scores.reshape(b, topK, 1)

    # cbb = torch.cat([xs, ys, wh], dim=-1)
    half_w, half_h = wh[..., 0:1] / 2, wh[..., 1:2] / 2
    bb = torch.cat([xs - half_w, ys - half_h, xs + half_w, ys + half_h], dim=-1)
    return bb, scores, clses


def get_detects(pred_hm, pred_wh, pred_offset, th, img_shape, topK=40, norm_wh=True, return_hm=False):
    h, w = img_shape
    b, c, output_h, output_w = pred_hm.shape

    bboxes, scores, clses = get_bboxs(pred_hm, pred_wh, pred_offset, topK, norm_wh)

    detects = []
    for batch in range(b):
        mask = scores[batch].gt(th)
        ...
        detects.append([batch_boxes, batch_scores, batch_clses,
                        pred_hm[batch] if return_hm else None])
    return detects
